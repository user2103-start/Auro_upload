...
OAuth state stored as simple filesystem tokens (no pickling)."""
...
def create_oauth_state() -> str:
    """Generate and store OAuth state as filesystem token.
    
    FIX: Store only state string, not Flow object.
    State is just a random token - file existence = valid state.
    """
    ...
    try:
        state_file.touch()
        logger.info(f"[OAUTH_STATE] Created state token: {state[:20]}...")
        logger.info(f"[OAUTH_STATE] File: {state_file}")
        return state
    except Exception as e:
        logger.error(f"[OAUTH_STATE] Failed to create state file: {type(e).__name__}: {e}")
        raise


def validate_oauth_state(state: str) -> bool:
    """Validate state exists and consume it.
    
    FIX: Minimal validation - just check file existence.
    File existence proves this state was generated by us.
    Delete file after validation (one-time use).
    """
    state_file = OAUTH_STATE_DIR / f"{state}.txt"
    
    if not state_file.exists():
        logger.warning(f"[OAUTH_STATE] State not found: {state[:20]}...")
        logger.warning(f"[OAUTH_STATE] Expected file: {state_file}")
        logger.warning(f"[OAUTH_STATE] Available states: {list(OAUTH_STATE_DIR.glob('*.txt'))}")
        return False
...
def create_oauth_flow() -> Flow:
    """Create a fresh OAuth Flow instance.
    
    FIX: Create Flow on-demand without storing it.
    Flow is recreated in callback using same config.
    """
    try:
...
class YouTubeOAuth:
    ...
    def get_authorization_url(self) -> Tuple[str, str]:
        ...
        FIX: Store only state token, not Flow object.
        """
        try:
            ...
            create_oauth_state()
            logger.info(f"[OAUTH] State persisted to filesystem as token")
    ...
    def handle_callback(self, code: str, state: str) -> bool:
        ...
        FIX: Validate state from filesystem, then recreate Flow.
        """
        try:
            ...
            logger.info(f"[CALLBACK] State validated, recreating Flow for token exchange")
            flow = create_oauth_flow()
